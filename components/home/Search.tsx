...
import { PetCardProps, PetProps } from "@/components/home/Props";
import Cookies from "js-cookie";
import { JsxElement } from "typescript";

async function fetchData(): Promise<PetProps[]> {
  try {
...
const Search = (props: any) => {
  ...
  const [dataValue, setDataValue] = useState(Cookies.get("data") || "");
  ...
  }, [pets, selectedAnimal, selectedBreed, selectedAge]);

  useEffect(() => {
    setTokenValue(Cookies.get("token") || "");
    setDataValue(Cookies.get("data") || "");
    console.log("run");
    if (pets.length > 0) {
      sortPets();
    }
  ...
  function checkSaved(id: number) {
    if (dataValue) return JSON.parse(dataValue).saved_pets.includes(id);
    return false;
  }
  ...
  // useEffect(() => {
  //   // pets.forEach(pet => sortPets(pet));
  //   sortPets();
  // }, [pets, sessionStorage.getItem("data")])
  ...
  function sortPets() {
    ...
    let petsArray: JSX.Element[] = [];
    pets.forEach((pet) => {
      let testingSelectedAnimal = selectedAnimal.trim();
      if (selectedAnimal.indexOf(")") !== -1) {
        testingSelectedAnimal = testingSelectedAnimal
          .substring(0, testingSelectedAnimal.indexOf("("))
          .trim();
      }
      testingSelectedAnimal = testingSelectedAnimal.substring(
        0,
        testingSelectedAnimal.lastIndexOf("s")
      );

      console.log("Testing Selected Animal: " + testingSelectedAnimal);
      if (
        (selectedAnimal === "Species (Any)" ||
          testingSelectedAnimal === pet.species) &&
        ...
        petsArray.push(
          <div key={pet.id} className="flex justify-center">
            <PetCard
              ...
              id={pet.id}
              url={pet.url}
              saved={checkSaved(pet.id)}
            />
          </div>
        );
      }
      setSortedPets(petsArray);
    });
